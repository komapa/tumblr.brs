...
'
'Steps:
'    1 - Get Tumblr reg code
'    2 - Send user to Tumblr web page to link Roku
'    3 - Retrieve Tumblr temporary token and temporary secret
'    4 - Enroll user in oauth/LOLOMO using NCCP 2.4 Web API enroll
'    5 - Retrieve Tumblr permanent token, user id and permanent secret
'Returns:
...
Function doOauthLink() As Integer
    status = doTempLink()
    if status=0
        ' busyDlg = ShowBusy("enrolling your Tumblr account",Dispatcher().port) ' optional
        status = doTumblrEnroll()
...
Function doTempLink() As Integer
    ...
    http = NewHttp("http://www.tumblr.com/oauth/request_token")
    oa.sign(http,false)
...
Function doTumblrEnroll() As Integer
    print "RegScreen: doTumblrEnroll"
    status = 1 ' error
    ...
    screen.AddHeaderText("Authorize Your Tumblr Account")
    ...
    screen.AddParagraph("http://www.tumblr.com/roku/activate")
...
Function doLink() As Integer
    ...
    pin.SetTitle("Enter PIN received from Tumblr on your desktop")
    pin.AddButton(0, "OK")
    ...
    http = NewHttp("http://www.tumblr.com/oauth/access_token")
